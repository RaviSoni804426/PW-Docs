...
| `single-instance-launch-wedge.patch` | Makes clicking the icon open the app when the previous instance is wedged. `sendMessage()` used a blocking `SendMessage`, which never returns against a process that holds the mutex but no longer pumps messages — so the launch hung with no window and never exited, and every further click stacked up another stuck process. Same fix PW Office already carries. |
| `projicons-clear-wedged-instance.patch` | The shippable half of the above: the `projicons` launcher clears wedged leftovers before starting `editors.exe`, so the icon works without a full core/CEF rebuild. Never touches a process that is showing a window, or one younger than 90 s. |
